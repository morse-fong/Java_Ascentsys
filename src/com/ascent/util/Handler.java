...
public class Handler extends Thread implements ProtocolPort {
			...
			while (!done)
			{
				...
				switch (opCode)
				{
					...
					case ProtocolPort.OP_ADD_USERS:
						...
						break;
					// 删除用户
					case ProtocolPort.OP_DEL_USERS:
						opDELUser();
						break;
					// 修改对应的用户数据
					case ProtocolPort.OP_CHANGE_USERS:
						opChangeUser();
						break;
					// 添加商品
					...
					case ProtocolPort.OP_GET_ALL_PRODUCTS:
						...
						break;
					// 修改对应的商品数据
					case ProtocolPort.OP_CHANGE_PRODUCT:
						opChangeProduct();
						break;
					// 删除对应的商品
	 ...
	 * 处理用户删除
	 */
	public void opDELUser()
	{
		try
		{
			// 从io读取到输入流对象中获取用户信息并创建用户对象
			User user = (User) this.inputFromClient.readObject();
			this.myProductDataAccessor.deleteUser(user);
		}
		catch (IOException | ClassNotFoundException e)
		{
			log("发生异常:  " + e);
	 ...
	 * 处理用户修改
	 */
	private void opChangeUser() {
		try
		{
			// 从io读取到输入流对象中获取用户信息并创建用户对象
			User old = (User) this.inputFromClient.readObject();
			User now = (User) this.inputFromClient.readObject();
			this.myProductDataAccessor.productUser(old,now);
		}
		catch (IOException | ClassNotFoundException e)
		{
			log("发生异常:  " + e);
