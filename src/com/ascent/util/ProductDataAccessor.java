...
public class ProductDataAccessor extends DataAccessor {
     ...
     * 商品信息数据文件名
     */
    protected static final String PRODUCT_FILE_NAME = "B:\\java_1\\a2\\src\\product.db";

    /**
     * 用户信息数据文件名
     */
    protected static final String USER_FILE_NAME = "B:\\java_1\\a2\\src\\user.db";
    ...
    public void deleteUser(User user) {
        ...
        try {
            ...
            String pre=user.getUsername() + "," + user.getPassword() + "," + user.getAuthority()+"\r\n";
            System.out.println(pre);
            //修改对应字符串内容
            str = str.replace(pre, "");
            System.out.println(str);
            //再把新的内容写入文件
            out = new PrintWriter(USER_FILE_NAME);
            out.write(str);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
    ...
    public void productUser(User pre, User now) {//创建文件
        ...
        try {
            ...
            String str = new String(fileContext, StandardCharsets.UTF_8);
            //修改对应字符串内容
            String spre=pre.getUsername() + "," + pre.getPassword() + "," + pre.getAuthority();
            String snow=now.getUsername() + "," + now.getPassword() + "," + now.getAuthority();
            str = str.replace(spre, snow);
            //再把新的内容写入文件
            out = new PrintWriter(USER_FILE_NAME);
            out.write(str);
    ...
    public HashMap<String, Product> getAllProducts() {
        // 加载文件
        this.load();
        HashMap<String, Product> productTable = new HashMap<String, Product>();
    ...
    public void deleteProduct(Product product) {
        ...
        productArrayList.removeIf(p -> p.isEqual(product));            // 删除
